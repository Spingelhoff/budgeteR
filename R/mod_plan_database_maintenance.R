#' plan_database_maintenance UI Function
#'
...
mod_plan_database_maintenance_ui <- function(id){
  ns <- NS(id)
  ...
  tagList(
    ...
    fluidRow(
      column(
        12,
        align = "center",
        textInput(ns("transaction_classification"), "Classification")
      )
    ),
    fluidRow(
      column(
...
#'
#' @noRd
mod_plan_database_maintenance_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    observeEvent(input$confirm_transaction_button, {
      ...
      sql <- "
        INSERT INTO ?table (date, classification, total)
        VALUES(?date, ?classification, ?total);
      "
      query <- DBI::sqlInterpolate(
        con,
        sql,
        table = input$table_selection,
        date = as.character(input$transaction_date),
        classification = input$transaction_classification,
        total = input$transaction_total
      )
      DBI::dbSendStatement(
        con,
        query
